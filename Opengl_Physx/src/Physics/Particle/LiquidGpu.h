...
public:
    ...
    float spacing = .2f;
    explicit LiquidGpu(PhysicsWorld& world, bool defaultWater = true) :world_(world), cuda_(*world.GetCuda()), shader_("Assets/Shaders/liquid_particles.glsl")
    {
        try {
            ...
            containerEnabled_ = defaultWater;
            if (defaultWater) { SetContainer(containerPosition_, containerSize_); Reset(); }
        }
        catch (...) { Release(); throw; }
    }
    ~LiquidGpu() { Release(); }
    ...
    void Reset()
    {
        ...
        try {
            ...
            if (reuse)
            {
                ...
                GL::BindBuffer(GL::ArrayBuffer, vbo_); GL::BufferData(GL::ArrayBuffer, 2 * MaxParticles * sizeof(PxVec4), nullptr, 0x88E8); GL::BindBuffer(GL::ArrayBuffer, 0);
    ...
    unsigned Emit(glm::vec3 origin, glm::vec3 direction, float speed, float radius, unsigned requested, float duration)
    {
        ...
        if (!particles_) {
            ...
            GL::BufferData(GL::ArrayBuffer, 2 * MaxParticles * sizeof(PxVec4), nullptr, 0x88E8);
        ...
        const float inverseMass = 1.0f / (10.0f * simulationSpacing_ * simulationSpacing_ * simulationSpacing_);
    ...
    void Draw(const Camera& camera, int width, int height)
    {
        ...
        renderTimer_.End();
        if (containerEnabled_ && showDebugBounds_) {
