...
public:
    ...
    GLuint Render(GLuint particleVao, GLuint screenVao, unsigned count, float radius, const glm::mat4& projection, const glm::mat4& view, GLuint sceneColor, GLuint sceneDepth, int w, int h, glm::vec3 low, glm::vec3 high, unsigned long long revision = std::numeric_limits<unsigned long long>::max()) {
        Allocate(w, h);
        const glm::vec3 center = (low + high) * .5f, light = SceneLight::Direction();
        const float span = std::max(glm::length(high - low) * .65f, 4.f);
        const glm::vec3 up = std::abs(light.y) > .99f ? glm::vec3(0, 0, 1) : glm::vec3(0, 1, 0);
        auto lightView = glm::lookAt(center + light * span * 2.f, center, up);
        ...
        const bool rebuild = !cacheable || !cached_ || light != cachedLight_ || revision != cachedRevision_ || count != cachedCount_ || radius != cachedRadius_ || low != cachedLow_ || high != cachedHigh_;
        if (rebuild) {
            ...
            cachedLight_ = light; cached_ = cacheable; cachedRevision_ = revision; cachedCount_ = count; cachedRadius_ = radius; cachedLow_ = low; cachedHigh_ = high;
        }
