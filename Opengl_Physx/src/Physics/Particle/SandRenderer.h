...
public:
    struct Parameters {
        glm::vec3 color{ .68f,.49f,.27f };
        float roughness=.38f, sparkle=1.2f, mineralFraction=.20f;
        float microScale=6.f, occlusion=1.f;
        float sunAzimuth=-155.f, sunElevation=35.f;
    };
    ...
    SandRenderer():shader_("Assets/Shaders/sand_granular.glsl",{"DEPTH","SHADE"}) {
        ...
        GL::GenVertexArrays(1,&vao_); GL::GenFramebuffers(1,&fbo_);glGenTextures(2,textures_);
    }
    ~SandRenderer(){GL::DeleteVertexArrays(1,&vao_);GL::DeleteFramebuffers(1,&fbo_);glDeleteTextures(2,textures_);}
    ...
    void Draw(GLuint buffer,size_t idOffset,unsigned count,float radius,const Camera& camera,int width,int height,Parameters p) {
        ...
        if(width!=width_||height!=height_){
            for(int i=0;i<2;++i){
                GL::ActiveTexture(0x84C0+14+i);glBindTexture(GL_TEXTURE_2D,textures_[i]);
                glTexImage2D(GL_TEXTURE_2D,0,i==0?0x8CAC:0x881A,width,height,0,i==0?GL_DEPTH_COMPONENT:GL_RGBA,GL_FLOAT,nullptr);
                ...
                GL::FramebufferTexture2D(0x8D40,i==0?0x8D00:0x8CE0,GL_TEXTURE_2D,textures_[i],0);
            }
        ...
        for(int i=0;i<2;++i){GL::ActiveTexture(0x84C0+14+i);glBindTexture(GL_TEXTURE_2D,0);}
        ...
        common("DEPTH");shader_.SetVector2("viewportOrigin",glm::vec2(0));instanced_(GL_TRIANGLE_STRIP,0,4,count);
        ...
        for(int i=0;i<2;++i){GL::ActiveTexture(0x84C0+14+i);glBindTexture(GL_TEXTURE_2D,textures_[i]);}
        shader_.SetInt("grainDepth",14);shader_.SetInt("grainNormals",15);
...
private:
    struct State {
        GLint readFbo{},drawFbo{},active{},textures[2]{},viewport[4]{},depthFunc{},program{},vao{},buffer{};
        ...
        State(){
            ...
            for(int i=0;i<2;++i){GL::ActiveTexture(0x84C0+14+i);glGetIntegerv(GL_TEXTURE_BINDING_2D,&textures[i]);}
        ...
        ~State(){
            ...
            for(int i=0;i<2;++i){GL::ActiveTexture(0x84C0+14+i);glBindTexture(GL_TEXTURE_2D,textures[i]);}GL::ActiveTexture(active);
            GL::BindVertexArray(vao);GL::BindBuffer(GL::ArrayBuffer,buffer);GL::UseProgram(program);
        }
    };
    Shader shader_;GLuint vao_{},fbo_{},textures_[2]{};int width_{},height_{};
