...
private:
    ...
    void Build(const Scene& scene, const BlastScene* blast, int width, int height)
    {
        ...
        copyText = line;
        std::snprintf(line,sizeof(line),"Rigid %zu | Soft %zu | Blast %u / Chunks %u / Bonds %u | Water %u / Sand %u\n",
            scene.GetBodyCount(),scene.GetSoftBodyCount(),blast?blast->GetActorCount():0,blast?blast->GetVisibleChunkCount():0,
            blast?blast->GetBondCount():0,water?water->Count():0,sand?sand->Count():0);
        copyText += line;
